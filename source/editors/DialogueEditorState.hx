...
class DialogueEditorState extends MusicBeatState
{
	...
	function addDialogueLineUI() {
		...
		sizeStepper = new FlxUINumericStepper(characterInputText.x + characterInputText.width + 5, characterInputText.y, 0.1, 0.7, 0.1, 2, 2);
		...
		tab_group.add(new FlxText(sizeStepper.x, sizeStepper.y - 18, 0, 'Text Size:'));
	...
	function reloadText(speed:Float = 0.05, size:Float = 0.7) {
		if(daText != null) {
			daText.killTheTimer();
			daText.kill();
			remove(daText);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			if(sender == speedStepper) {
				...
				dialogueFile.dialogue[curSelected].textSize = sizeStepper.value;
				if(Math.isNaN(dialogueFile.dialogue[curSelected].textSize) || dialogueFile.dialogue[curSelected].speed == null || dialogueFile.dialogue[curSelected].speed < 0.1) {
					dialogueFile.dialogue[curSelected].speed = 0.7;
				}
			}
			reloadText(speedStepper.value, sizeStepper.value);
		}
	}
	...
	function changeText(add:Int = 0) {
		...
		sizeStepper.value = curDialogue.textSize;

		curAnim = 0;
		character.reloadCharacterJson(characterInputText.text);
		reloadCharacter();
		updateTextBox();
		reloadText(curDialogue.speed, curDialogue.textSize);
